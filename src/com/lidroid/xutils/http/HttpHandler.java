...
public class HttpHandler<T> extends PriorityAsyncTask<Object, Object, Void>
	...
	public HttpHandler(AbstractHttpClient client, HttpContext context,
		...
		if (callback == null) {
			throw new RuntimeException("Http RequestCallBack must not be null!");
		}
	}
	...
	// 执行请求,
	// 可能返回: responseInf<File> 或 responseInfo<String>
	private ResponseInfo<Object> sendRequest(HttpRequestBase request)
	 ...
	 * @Author liuzw 检索缓存内容
	 */
	private ResponseInfo<String> checkCache(HttpRequestBase request) {
		requestMethod = request.getMethod();
		if (HttpUtils.sHttpCache.isEnabled(requestMethod)) {
			String result = HttpUtils.sHttpCache.get(requestUrl);
			if (result != null) {
				return new ResponseInfo<String>(null, result, true);
			}
		}
		return null;
	}
	...
	protected Void doInBackground(Object... params) {
		...
		try {
			...
			if (!isDownloadingFile) {
				responseInfo = checkCache(request);
				if (responseInfo != null) {
					try {
						dT = callback.onParseData((String) responseInfo.result);
					} catch (Exception exception) {
					}
					this.publishProgress(UPDATE_CACHE, responseInfo);
				}
			}

			ResponseInfo<?> responseInfo2 = sendRequest(request);
			if (responseInfo2 == null && responseInfo != null) {
				// 如果从网络请求数据失败,则复用缓存内容.
				// responseInfo2 = responseInfo;
				this.publishProgress(UPDATE_FINISHED, responseInfo);
				return null;
			}
			responseInfo = responseInfo2;
			if (responseInfo != null) {
				// 将解析过程融入线程中来, 比如解析10Kjson需要10S钟,则能够节省UI卡顿10s.
				dT = callback.onParseData((String) responseInfo.result);
	...
	private final static int UPDATE_CACHE = 5;
	private final static int UPDATE_FINISHED = 6;
	...
	protected void onProgressUpdate(Object... values) {
		...
		case UPDATE_FAILURE: {
			...
			this.publishProgress(UPDATE_FINISHED, null);
			break;
		}
		case UPDATE_SUCCESS: {
			onSuccess(false, values);
			break;
		}
		case UPDATE_CACHE: {
	...
	// 分发成功事件进行相应的回调.
	public void onSuccess(boolean isFromCache, Object... values) {
		this.state = State.SUCCESS;

		if (values.length != 2) {// 如果参数校验有问题,则不成功也无失败,应该调用Finished.否则事件体系不完整.
			this.publishProgress(UPDATE_FINISHED, null);
			return;
		}
		ResponseInfo<Object> data = (ResponseInfo<Object>) values[1];
		if (data != null && data.result != null) {
			if (data.result instanceof String) {

				// 如果转换成功则调用onSuccess(T data);
				// 怎么样在background中调用.这样可以节省解析数据的时间.
				T dT = getParsedResponseData(); //

				// 三种success 只需要实现其中一个就行,不要同时实现多个success.
				if (dT != null) {
					callback.onSuccess(dT, isFromCache);
					data.result = dT;// 当实现了数据解析后,相应的内容也要变成相应的类型.
				}

				callback.onSuccess(data);
				callback.onSuccess(data, isFromCache);

			} else if (data.result != null && data.result instanceof File) {
				callback.onSuccess(data);
				callback.onSuccess(data, isFromCache);
				callback.onSuccess((T) data.result, isFromCache);
			}
	...
	private T getParsedResponseData() {
		return dT;
	}

	private ResponseInfo<Object> handleResponse(HttpResponse response)
		...
		if (statusCode < 300) {
			...
			return new ResponseInfo<Object>(response, result, false);
