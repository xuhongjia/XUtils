...
import com.lidroid.xutils.db.sqlite.IDatabase;
...
public class Table {
    ...
    private Table(IDatabase db, Class<?> entityType) {
        this.db = db;
    ...
    public static synchronized Table get(IDatabase db, Class<?> entityType) {
    ...
    public static synchronized void remove(IDatabase db, Class<?> entityType) {
        String tableKey = db.getDaoConfig().getDbName() + "#" + entityType.getName();
        tableMap.remove(tableKey);
    }

    public static synchronized void remove(IDatabase db, String tableName) {
        if (tableMap.size() > 0) {
            String key = null;
